...
def all_experiments(path, accept=None):
    experiments = []
    for file in os.listdir(path):
        if file[-5:] == ".json":
            experiment = load_experiment(path, file)
            if accept is None or accept(experiment):
...
class Experiment():
    ...
    def identifier(self):
        tmp = "{}_{}".format(self.data["title"], self.data["hash"][:8])
